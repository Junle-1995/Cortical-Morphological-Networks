function Results = gretna_prediction_all_var_for_pca_cov(Predictors, Responses, Cov, K_fold, C_type, C_thr, R_type, Ncomp, Lambda)
...
if nargin > 9
    error('At most 8 arguments are allowed!');
end
...
Responses_cov_regress    = zeros(size(Responses));
...
for ifold = 1:K_fold
    ...
    for ivar = 1:Nvar_res
        ...
        [b,~,r] = regress(Responses_train,[ones(length(Responses_train),1),Cov_train]);
        Responses_train = b(1) + r;
        Responses_test  = Responses(Idx_test,ivar) - Cov_test*b(2:end);
        Responses_cov_regress(Idx_test,ivar) = Responses_test;
...
for ivar = 1:Nvar_res
    if sum(~isnan(Responses_predicted_test(:,ivar))) >= 2
        [r,p]     = corr(Responses_cov_regress(:,ivar),Responses_predicted_test(:,ivar),'rows','pairwise');
        R(ivar,1) = r;
        P(ivar,1) = p;
    else
...
Results.R                  = R;
Results.P                  = P;
Results.Responses_regress  = Responses_cov_regress;
